...
import { c6Transactions } from '@/lib/c6-data';
...
export default function C6Sync({ onClose }: { onClose: () => void }) {
  ...
  const missingTransactions = useMemo(() => {
    ...
    return c6Transactions.filter(c6Tx => {
  ...
  const handleUndoSync = async () => {
    ...
    const importedTransactions = dbTransactions.filter(tx => tx.descricao.endsWith('(Importado C6)'));
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Encontramos {c6Transactions.length - missingTransactions.length} transações do extrato que já constam no app.
            </p>
            {c6Transactions.length - missingTransactions.length > 0 && (
